...
Ws_AMOLED::Ws_AMOLED() : boards(NULL)
{
    pBuffer = NULL;
...
uint8_t Ws_AMOLED::getBoardID()
{
    if (boards == &BOARD_AMOLED_147)
    {
        return Ws_AMOLED_147;
    }
    else if (boards == &BOARD_AMOLED_191)
    {
        return Ws_AMOLED_191;
    }
    else if (boards == &BOARD_AMOLED_241)
    {
        return Ws_AMOLED_241;
    }
    return Ws_AMOLED_UNKNOWN;
}

const BoardsConfigure_t *Ws_AMOLED::getBoarsdConfigure()
{
    return boards;
}

uint16_t Ws_AMOLED::width()
{
    return _width;
}

uint16_t Ws_AMOLED::height()
{
    return _height;
}

void inline Ws_AMOLED::setCS()
{
    digitalWrite(boards->display.cs, LOW);
}

void inline Ws_AMOLED::clrCS()
{
    digitalWrite(boards->display.cs, HIGH);
}

bool Ws_AMOLED::isPressed()
{
    if (boards == &BOARD_AMOLED_147)
    {
...
uint8_t Ws_AMOLED::getPoint(int16_t *x, int16_t *y, uint8_t get_point)
{
    uint8_t gesture = 0;
    uint8_t point = 0;
    if (boards == &BOARD_AMOLED_147)
    ...
    {
        point = myTouch.getTouch(x, y, &gesture);
    }
    return point;
}

uint16_t Ws_AMOLED::getBattVoltage(void)
{
    if (boards)
    {
        if (boards->pmu)
...
uint16_t Ws_AMOLED::getVbusVoltage(void)
{
    if (boards)
    {
        if (boards->pmu)
...
bool Ws_AMOLED::isBatteryConnect(void)
{
    if (boards)
    {
        if (boards->pmu)
...
uint16_t Ws_AMOLED::getSystemVoltage(void)
{
    if (boards)
    {
        if (boards->pmu)
...
bool Ws_AMOLED::isCharging(void)
{
    if (boards)
    {
        if (boards->pmu)
...
static uint32_t deviceScan(TwoWire *_port, Stream *stream)
{
...
bool Ws_AMOLED::initBUS()
{
    ...
    spi_bus_config_t buscfg = {
        ...
        .flags = SPICOMMON_BUSFLAG_MASTER | SPICOMMON_BUSFLAG_GPIO_PINS, // 主机模式下初始化 , 不是默认SPI总线GPIO
    };
...
bool Ws_AMOLED::beginAutomatic()
{
    return begin();
}

bool Ws_AMOLED::beginAMOLED_191(bool touchFunc)
{
...
bool Ws_AMOLED::beginAMOLED_241()
{
    ...
    {
        Wire.begin(boards->touch->sda, boards->touch->scl);
        deviceScan(&Wire, &Serial); // 查找I2C的从机
        // Try to find touch device
        Wire.beginTransmission(0x38);
        if (Wire.endTransmission() == 0)
        {
            // TouchDrvCSTXXX::setPins(boards->touch->rst, boards->touch->irq);
            bool res = TouchDrvFT6X36::begin(Wire, 0x38, boards->touch->sda, boards->touch->scl);
            if (!res)
            {
                Serial.print("Failed to find FT6336 - check your wiring!");
            }
            else
            {
                _touchOnline = true;
 ...
 */
bool Ws_AMOLED::installSD(int miso, int mosi, int sclk, int cs)
{
...
void Ws_AMOLED::uninstallSD()
{
    SD.end();
}

bool Ws_AMOLED::beginAMOLED_147()
{
...
void Ws_AMOLED::writeCommand(uint32_t cmd, uint8_t *pdat, uint32_t lenght) // 5100 data 1 02 005100
{
    setCS();
    spi_transaction_t t;
    memset(&t, 0, sizeof(t));
    t.flags = (SPI_TRANS_MULTILINE_CMD | SPI_TRANS_MULTILINE_ADDR); // 默认使用了一线数据线,没有说明是4线的话
    t.cmd = 0x02;
    t.addr = cmd;
    if (lenght != 0)
    {
...
void Ws_AMOLED::setBrightness(uint8_t level)
{
    _brightness = level;
    lcd_cmd_t t = {0x5100, {level}, 0x01}; // ADDR BUF LEN
    writeCommand(t.addr, t.param, t.len);
}

uint8_t Ws_AMOLED::getBrightness()
{
    return _brightness;
}

void Ws_AMOLED::setAddrWindow(uint16_t xs, uint16_t ys, uint16_t xe, uint16_t ye)
{
    xs += _offset_x;
    ys += _offset_y;
    xe += _offset_x;
    ye += _offset_y;
    lcd_cmd_t t[3] = {
        {0x2A00, {(uint8_t)((xs >> 8) & 0xFF), (uint8_t)(xs & 0xFF), (uint8_t)((xe >> 8) & 0xFF), (uint8_t)(xe & 0xFF)}, 0x04},
        {0x2B00, {(uint8_t)((ys >> 8) & 0xFF), (uint8_t)(ys & 0xFF), (uint8_t)((ye >> 8) & 0xFF), (uint8_t)(ye & 0xFF)}, 0x04},
        {0x2C00, {0x00}, 0x00},
    };
...
void Ws_AMOLED::pushColors(uint16_t *data, uint32_t len)
{
...
void Ws_AMOLED::pushColors(uint16_t x, uint16_t y, uint16_t width, uint16_t hight, uint16_t *data)
{
...
void Ws_AMOLED::attachPMU(void (*cb)(void))
{
    if (boards)
    {
...
void Ws_AMOLED::enablePMUInterrupt(uint32_t params)
{
    if (boards)
    {
        if (boards->pmu && (boards == &BOARD_AMOLED_147))
...
void Ws_AMOLED::diablePMUInterrupt(uint32_t params)
{
    if (boards)
    {
        if (boards->pmu && (boards == &BOARD_AMOLED_147))
...
void Ws_AMOLED::wakeup()
{
    lcd_cmd_t t = {0x1100, {0x00}, 1}; // Sleep Out
    writeCommand(t.addr, t.param, t.len);
}

bool Ws_AMOLED::hasTouch()
{
...
void Ws_AMOLED::setRotation(uint8_t rotation)
{
    uint8_t data = 0x00;
    rotation %= 4;
    _rotation = rotation;
...
uint8_t Ws_AMOLED::getRotation()
{
    return (_rotation);
}

bool Ws_AMOLED::needFullRefresh()
{
    if (boards)
    {
